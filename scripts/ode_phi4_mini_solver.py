#!/usr/bin/env python3
"""Solver standalone per equazioni differenziali con SymPy e Ollama phi4-mini.

Uso rapido:
  python scripts/ode_phi4_mini_solver.py --equation "y' = y" --ics "y(0)=1"
  python scripts/ode_phi4_mini_solver.py --equation "y'' + y = 0" --ics "y(0)=0, y'(0)=1"
...
Note:
  ...
  - phi4-mini spiega il risultato o il limite incontrato.
  - Non usa bus agentici, database locali o container.
"""
...
def solve_with_sympy(equation: Any, sp: Any, yx: Any, ics: dict[Any, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        result["solution"] = sp.dsolve(equation, yx, **kwargs)
    except Exception as exc:
...
def parse_numeric_bound(raw: str, default: float, sp: Any, transformations: Any) -> float:
    if not raw.strip():
        return default
    local_dict = build_local_dict(sp, sp.Symbol("x"), sp.Function("y"))
    value = parse_expression(raw.replace("^", "**"), sp, transformations, local_dict)
...
def plot_solution(
    ...
    *,
    ...
    if unresolved:
        print("\n=== Grafico ===")
        print("Grafico non generato: la soluzione contiene simboli senza valore: " + ", ".join(unresolved))
...
def ollama_generate(host: str, prompt: str, timeout: int) -> str:
...
def build_phi4_mini_prompt(
    ...
    return textwrap.dedent(
        ...
        Non descrivere te stesso come un modello progettato per usare SymPy.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Risolvi equazioni differenziali con SymPy e spiega con Ollama phi4-mini."
    )
    ...
    parser.add_argument("--no-ollama", action="store_true", help="Non chiamare phi4-mini.")
...
def main() -> int:
    ...
    if not args.no_ollama:
        ...
        sys.stdout.flush()
        prompt = build_phi4_mini_prompt(raw_equation, normalized, params, ics, result)
        try:
            response = ollama_generate(args.ollama_host, prompt, args.ollama_timeout)
            print_llm_response(response, result)
        except RuntimeError as exc:
            print(exc)
            print(f"Se il modello manca, prova: ollama pull {PHI4_MINI_MODEL}")
